// DescriptiveStatistics.cpp by Ulrich Eisenecker, July 22, 2025
...
[[nodiscard]] auto generateRandomData(DISTRIBUTION&& distribution,
                                      GENERATOR&& generator,
                                      std::size_t n)
/* Passing distribution and generator as r-value references allows
 * to use variable and temporaries, but not const objects.
 */
{
...
[[nodiscard]] auto calculateParameters(std::ranges::forward_range auto&& data)
{
   ...
   auto median { n % 2 ? data[n / 2] : (data[n / 2 - 1] + data[n / 2]) / PRECISION { 2.0 } };
...
void outputParameters(std::ostream& os,const std::tuple<Pair...>& params)
{
   std::apply([&os](const Pair& ...param)
              { ((output(os,param) << std::endl), ...); },params);
}
...
int main()
{
   ...
   ranges::sort(femaleHeights);
   for (auto v : femaleHeights)
      cout << v << endl;
   cout << endl;
}
